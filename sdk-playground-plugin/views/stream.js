...
const docString = `### 🎬 Реальный код макета Stream
...
export function getMediaDoc() {
  if (state.mediaCode) {
    return `### 🎬 Настоящий код макета Stream
...
#### Исходный код файла (\`views/stream.js\`):
\`\`\`js
${state.mediaCode}
\`\`\`
`;
...
export function buildStreamCard(state, setMediaPlayerPlayback, setActiveFilterTracker, setActiveFilterQuality) {
  ...
  if (state.showMediaCode) {
    childrenList.push(Markdown(getMediaDoc()));
    childrenList.push(Divider());
  }
