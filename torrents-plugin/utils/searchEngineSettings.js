...
import { resolveSearchEngineApiKey, resolveSearchEngineUrl, searchEngineHeaders } from './config.js';
...
export function fileConfigToSettings(config) {
  return {
    ...
    proxyUrl: config.proxy.list[0]?.url ?? "",
...
export function settingsToFileConfig(settings) {
  ...
  return {
    ...
    animelayer: { enableSearch: !!settings.animelayerEnableSearch },
    ...
    kinozal: { enableSearch: !!settings.kinozalEnableSearch },
